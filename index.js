...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname && links[pathname]) {
      ...
      const currentFileTime = timeData.currentFileTime;
      const flooredTime = Math.floor(currentFileTime / 10000000);
      const hashRes = await fetch(`https://api.hashify.net/hash/md5/hex?value=${flooredTime}`);
