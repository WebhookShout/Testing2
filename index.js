...
export default {
  async fetch(request, env, ctx) {
    ...
    const enkey = url.searchParams.get("key"); // get key in '?key=mykey'
    ...
    if (pathname && auth) {
      ...
      const encoded = EncodeScript(content, String(enkey));
    ...
    if (pathname && links[pathname]) {
      ...
      const json = JSON.stringify({Name: randomName, Expiration: Date.now() + 5000});
      const enfcStr = GetRandomString(10);
      const fnctblStr = GetRandomString(9);
      ...
      const code = `loadstring("\\${encodeAscii(`local t={[1]=Instance,[2]="new",[3]="StringValue",[4]=game,[5]="GetService",[6]="ReplicatedStorage",[7]="Parent",[8]="Name",[9]="Archivable",[10]="Value",[11]=true,[12]=game:GetService("HttpService"):JSONDecode(game:HttpGet("https://api.hashify.net/hash/md5/hex?value="..math.floor(game:GetService("HttpService"):JSONDecode(game:HttpGet("http://worldclockapi.com/api/json/utc/now")).currentFileTime / 10000000))).Digest,[13]="${randomName}"} local v=t[1][t[2]](t[3])v[t[7]]=t[4][t[5]](t[4], t[6])v[t[8]]=t[13]v[t[9]]=t[11]v[t[10]]=t[12] ${antihookcode} loadstring(game:HttpGet("${domain}/${url.pathname.slice(1)}?auth=${EncodeText(json, ServiceKey)}&key="..game:GetService("ReplicatedStorage")["${randomName}"].Value))() ${enfcStr} = true`)}")()`;
