...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname) {
      ...
      const code = `loadstring("\\${encodeAscii(`${randomName} = tostring(os.time()):sub(1,9) print(${randomName}, ${secureKey}) ${antihookcode} loadstring(game:HttpGet("${domain}/${url.pathname.slice(1)}?auth=${EncodeText(json, ServiceKey)}"))() ${enfcStr} = true`)}")()`;
